...
export default function HomeClient({ patients }: { patients: Patient[] }) {
  ...
  useEffect(() => {
    if (window.localStorage.getItem("aftercare_admin_authed") === "true") {
      const frame = window.requestAnimationFrame(() => setAuthed(true));
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main" id={screen}>
        ...
        <footer className="page-foot"><span>Patient recovery operations</span><span>Decision support only — not for emergency use</span></footer>
        </>}
      </main>
      ...
      {modal && (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(null); }}>
          <section className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
            ...
            {modal === "integration" && <><span className="modal-kicker">Data sources</span><h2 id="modal-title">Connected health data</h2><p className="modal-copy">Manage the health and clinical sources available to the recovery workspace.</p><div className="integration-list"><div><span className="integration-icon apple">♥</span><p><strong>Apple Health</strong><small>Activity, sleep, heart rate</small></p><b>Active</b></div><div><span className="integration-icon whoop">W</span><p><strong>WHOOP</strong><small>Recovery, strain, sleep</small></p><b>Active</b></div><div><span className="integration-icon ehr">＋</span><p><strong>Clinic EHR</strong><small>Discharge notes, labs, appointments</small></p><b>Connected</b></div></div><div className="modal-actions"><button type="button" className="dark-button full" onClick={() => setModal(null)}>Done</button></div></>}
          </section>
        </div>
      )}
