        ...
        <div className="profile-content-grid">
          <div className="profile-primary">
            {activeTab === "Overview" && (
              <>
                ...
                <section className="profile-ai-card">
                  <div className="profile-card-heading"><div><span className="ai-mark">✦</span><div><h3>AI care brief</h3><p>Evidence assembled from the active record</p></div></div><span className="review-tag">Clinical review required</span></div>
            ...
            {activeTab === "Clinical record" && (
              <>
                ...
                <div className="clinical-columns">
                  <section className="profile-card clinical-card"><div className="profile-card-heading plain"><div><h3>Conditions</h3><p>Active and relevant history</p></div><button type="button" onClick={() => notify("Condition editor is disabled in demo mode")}>Edit</button></div>{patient.conditions.map((condition) => <div className="clinical-list-row" key={condition}><span className="condition-mark" /><strong>{condition}</strong><small>Confirmed record</small></div>)}</section>
                  <section className="profile-card clinical-card"><div className="profile-card-heading plain"><div><h3>Allergies</h3><p>Visible to the whole care team</p></div><button type="button" onClick={() => notify("Allergy editor is disabled in demo mode")}>Edit</button></div>{patient.allergies.map((allergy) => <div className="clinical-list-row" key={allergy}><span className="allergy-mark">!</span><strong>{allergy}</strong><small>Clinical record</small></div>)}</section>
                </div>
            ...
            {activeTab === "Documents & history" && (
              <>
                <section className="profile-card documents-card"><div className="profile-card-heading plain"><div><h3>Patient documents</h3><p>Records attached to the current recovery episode</p></div><button type="button" onClick={() => notify("Upload is disabled for this synthetic demo")}>＋ Add document</button></div><div className="document-grid">{patient.documents.map((document) => <button key={document.name} type="button" onClick={() => notify(`${document.name} opened in preview mode`)}><span className="document-icon">▤</span><div><strong>{document.name}</strong><small>{document.type} · {document.date}</small></div><em>Open →</em></button>)}</div></section>
                <section className="profile-card history-card"><div className="profile-card-heading plain"><div><h3>Complete activity history</h3><p>Patient, data-source, AI, and staff events in one traceable log</p></div><button type="button" onClick={() => notify("Audit trail exported")}>Export audit trail</button></div><div className="full-timeline">{patient.timeline.map((event, index) => <div className="full-timeline-row" key={`${event.time}-${event.title}`}><div className="timeline-rail"><span className={event.tone}>{event.tone === "critical" ? "!" : event.tone === "watch" ? "↗" : event.tone === "stable" ? "✓" : "•"}</span>{index < patient.timeline.length - 1 && <i />}</div><time>{event.time}</time><div><strong>{event.title}</strong><p>{event.detail}</p><small>{event.source}</small></div></div>)}</div></section>
              </>
            )}
          </div>
          ...
          <aside className="profile-aside">
            <section className="profile-card personal-record">
              <div className="profile-card-heading plain"><div><h3>Personal details</h3><p>Synthetic identity record</p></div><button type="button" onClick={() => notify("Personal detail editing is disabled in demo mode")}>Edit</button></div>
